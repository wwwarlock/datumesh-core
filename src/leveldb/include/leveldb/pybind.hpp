...
#ifndef PYBIND11_IMAGE_HPP_
#define PYBIND11_IMAGE_HPP_
...
/**
 * @file python/pybind11_Image.hpp
 * @brief Transparent conversion to and from Python for :core::Image.
 *
 * Numpy uses row-major storage order by default.
 * :core::Image uses col-major storage (like Eigen).
 * 
...
/**
 * @brief Transparent conversion for :core::Image3u to and from Python.
 *
 * Converts an :core::Image3u to and from Python. Can construct a :core::Image3u from numpy arrays,
 ...
 */
template<>
struct type_caster<core::Image3u>
{
    ...
    static handle cast(const :core::Image3u& src, return_value_policy /* policy */, handle /* parent */)
	{
    ...
    PYBIND11_TYPE_CASTER(:core::Image3u, _("numpy.ndarray[uint8[m, n, 3]]"));
};

/**
 * @brief Transparent conversion for :core::Image4u to and from Python.
 *
 * Converts an :core::Image4u to and from Python. Can construct a :core::Image4u from numpy arrays,
 * as well as potentially other Python array types.
 *
 * Note: Not sure what happens if the given numpy array is not contiguous, not in default (row-major) storage
 *       order, or has non-standard strides. It may or may not work.
 */
template<>
struct type_caster<:core::Image4u>
{
	bool load(handle src, bool)
	{
		auto buf = pybind11::array::ensure(src);
		...
		// Like with OpenCV: value = cv::Mat(buf.shape(0), buf.shape(1), opencv_type, buf.mutable_data());
		// But since it doesn't, we just copy the data for now:
		value = da::core::Image4u(buf.shape(0), buf.shape(1));
		array_t<std::uint8_t> buf_as_array(buf);
		for (int r = 0; r < buf.shape(0); ++r) {
			for (int c = 0; c < buf.shape(1); ++c) {
				value(r, c)[0] = buf_as_array.at(r, c, 0);
    ...
    static handle cast(const da::core::Image4u& src, return_value_policy /* policy */, handle /* parent */)
	{
    ...
    PYBIND11_TYPE_CASTER(da::core::Image4u, _("numpy.ndarray[uint8[m, n, 4]]"));
};
